...
void SelectTool::mousePressEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
    ...
    if( selectMode == sm_none ){
        selectMode = sm_netSelect;
        QList<QGraphicsView*> views  = scene->views();
        if ( views.count() > 0 ){
            QGraphicsView * view = views.first();
    ...
    if ( selectMode == sm_move && items.count() == 1 ){
        //清除旧的拓扑图对象与连接线对象
        if (dashRect ){
...
void SelectTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
    ...
    if ( items.count() == 1 ){
        ...
        if ( item != 0 ){
            ...
            if ( nDragHandle != None && selectMode == sm_size ){
                //Unused
            ...
            else if(nDragHandle == None && selectMode == sm_none ){
                //碰撞检测当前的矩形点位置
                 //SizeHandleRect::Direction handle = item->hitTest(event->scenePos());
                Direction handle = item->hitTest(event->scenePos());
    ...
    //多选时处理逻辑
    if ( (selectMode != sm_size  && items.count() > 1) || isGroup )
    {
        scene->mouseEvent(event);
    }
}
...
void SelectTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
    ...
    if ( items.count() == 1 ){
        ...
        if ( item != 0  && selectMode == sm_move && c_last != c_down ){
             item->setPos(c_last);
        }
    }

    //多选时，重置视图的拖拽状态
    if (selectMode == sm_netSelect ){
        QList<QGraphicsView*> views  = scene->views();
        if ( views.count() > 0 ){
            QGraphicsView * view = views.first();
    ...
    if ( items.count() == 1 ){
        ...
        if ( item != 0 ){
            ...
            if ( nDragHandle != None)
            {
                ...
                setCursor(scene,QCursor(QPixmap(":/images/rotation.png")));
            }
            else{
...
void RotationTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, DrawScene *scene)
{
    ...
    if ( items.count() == 1 ){
        GraphicsItem * item = qgraphicsitem_cast<GraphicsItem*>(items.first());
        if ( item != 0  && nDragHandle !=None && selectMode == sm_rotate ){
             QPointF origin = item->mapToScene(item->boundingRect().center());
