...
bool Crypto::rsaSign(string &orignal, string &sign_text_base64)
{
    if( !rsakeypair )
        return false;
    ...
    if( rsa ){
        ...
        //对待签名数据做SHA1摘要(此处理方法与Java中RSA签名的SHA1WithRSA模式匹配)
        SHA1(reinterpret_cast<const unsigned char*>(orignal.c_str()),static_cast<size_t>(orignal.length()), szTmp);
        unsigned int   nGetSign = 0;
        //unsigned char* pGetSign = new unsigned char [orignal.length()*2];
        unsigned char pGetSign[2048] = {'\0'};

        int ret = RSA_sign(NID_sha1,
                           szTmp,
                           static_cast<unsigned int>(strlen((char*)szTmp)),
                           pGetSign,&nGetSign,rsa);

        string sign_text(reinterpret_cast<char*>(pGetSign),nGetSign);
        sign_text_base64 = base64_encode(sign_text);
        RSA_free(rsa);
        //delete [] pGetSign;
        printf("rsaSign\n");
        return ret;
...
bool Crypto::rsaVerify(string &orignal, string &sign_text_base64)
{
    if( !rsakeypair )
        return false;
    ...
    if( rsa ){
        ...
        //对待签名数据做SHA1摘要(此处理方法与Java中RSA签名的SHA1WithRSA模式匹配)
        SHA1(reinterpret_cast<const unsigned char*>(orignal.c_str()),static_cast<size_t>(orignal.length()), szTmp);
        string sign_text = base64_decode(sign_text_base64);
        int ret = RSA_verify(NID_sha1,
                             szTmp,
                             static_cast<unsigned int>(strlen((char*)szTmp)),
                             reinterpret_cast<const unsigned char*>(sign_text.c_str()),
                             static_cast<unsigned int>(sign_text.length()),rsa);
...
bool Crypto::writeKeyToFile(FILE *file, int key)
{
